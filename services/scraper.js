...
const isValidOffer = (text) => {
  if (!text || typeof text !== 'string') return false;
  
  const cleanTxt = text.replace(/\s+/g, ' ').trim();
  if (cleanTxt.length < 8 || cleanTxt.length > 350) return false;
  
  const lower = cleanTxt.toLowerCase();

  // 1. Explicit UI Header & Button Blacklist
  const exactJunkHeaders = [
    'bank offers', 'card offers', 'available offers', 'offers for you', 
    'special offer', 'partner offers', 'best offers', 'coupons', 
    'promo code', 'view all offers', 'know more', 't&c', 'terms and conditions', 
    'terms & conditions apply', 'see all', 'apply', 'redeem', 'view details', 
    'details', 'more offers', 'check offer', 'explore', 'offer details', 'hide details'
  ];
  if (exactJunkHeaders.includes(lower)) return false;

  // 2. Junk Non-Offer Keywords Blacklist (Delivery, Return, Policy, Warranty, Seller)
  const junkPatterns = [
    'free delivery', 'standard delivery', 'delivered by', 'pay on delivery',
    'cash on delivery', '10 days returnable', '7 days replacement', 
    '14 days return', '100% original', 'gst invoice', 'warranty', '1 year warranty',
    '2 year warranty', 'in stock', 'out of stock', 'sold by', 'verified purchase',
  ...
  for (const junk of junkPatterns) {
    if (lower === junk || lower.startsWith(junk + ' ') || lower.endsWith(' ' + junk)) {
      const hasOfferMarkers = lower.includes('%') || lower.includes('₹') || lower.includes('$') || lower.includes('off') || lower.includes('discount') || lower.includes('cashback');
      if (!hasOfferMarkers) return false;
    }
  }

  if ((lower.includes('delivery') || lower.includes('return') || lower.includes('replacement') || lower.includes('warranty') || lower.includes('gst invoice')) &&
      !lower.includes('off') && !lower.includes('discount') && !lower.includes('cashback') && !lower.includes('%') && !lower.includes('₹') && !lower.includes('$') && !lower.includes('save')) {
    return false;
  }

  // 3. Acceptance Criteria: Must match Bank/Card terms OR Coupon/Discount terms
  const bankCardKeywords = [
    'bank', 'card', 'credit', 'debit', 'sbi', 'hdfc', 'icici', 'axis', 'kotak', 
    'bob', 'bank of baroda', 'idfc', 'amex', 'american express', 'onecard', 
    'indusind', 'hsbc', 'federal', 'au bank', 'citi', 'rupay', 'visa', 
    'mastercard', 'standard chartered', 'rbl', 'yes bank', 'canara', 'pnb', 
    'union bank', 'dbs', 'emi'
  ];

  const discountKeywords = [
    'off', 'discount', 'cashback', 'save', 'savings', 'coupon', 'code', 
    'promo', 'voucher', 'instant discount'
  ];

  const hasBankOrCard = bankCardKeywords.some(kw => lower.includes(kw));
  const cleanForCheck = lower.replace(/100%\s*original/g, '');
  const hasDiscountOrCode = discountKeywords.some(kw => cleanForCheck.includes(kw)) || cleanForCheck.includes('%') || cleanForCheck.includes('₹') || cleanForCheck.includes('$');

  if (!hasBankOrCard && !hasDiscountOrCode) {
    return false;
  }
...
const getCouponType = (text) => {
  const lower = text.toLowerCase();
  const bankCardKeywords = [
    'bank', 'card', 'credit', 'debit', 'sbi', 'hdfc', 'icici', 'axis', 'kotak', 
    'bob', 'idfc', 'amex', 'onecard', 'indusind', 'hsbc', 'federal', 'au bank', 
    'citi', 'rupay', 'visa', 'mastercard', 'rbl', 'yes bank', 'emi'
  ];
  if (bankCardKeywords.some(kw => lower.includes(kw))) {
    return 'bank_offer';
  }
  return 'product';
};
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (txt && txt.trim()) {
        ...
        if (isValidOffer(cleanTxt) && !coupons.some(c => c.description === cleanTxt)) {
          coupons.push({
            code: '',
            description: cleanTxt,
            couponType: getCouponType(cleanTxt),
